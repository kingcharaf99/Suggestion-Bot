     ...
     try {
      ...
      if(suggestionmessage.embeds[0].title !== `New Suggestion`) {
          ...
          if(suggestionmessage.embeds[0].title === `Suggestion Approved`){
                   ...
                   return message.channel.send(new Discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${message.client.emoji.fail} | The following suggestion was already approved`).setFooter('https://discord.gg/ChuNqz5z2q').setTimestamp().setColor('RED'));
  
          } else {
  
                   return message.channel.send(new Discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${message.client.emoji.fail} | The following suggestion was already Declined`).setFooter('https://discord.gg/ChuNqz5z2q').setTimestamp().setColor('RED'));
